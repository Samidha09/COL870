...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.att_embeddings = nn.Linear(self.input_dim - self.pe_dim, self.hidden_dim)

        self.att_embeddings_nope = nn.Linear(self.input_dim, self.hidden_dim)

        self.pe_embeddings = nn.Linear(self.pe_dim, self.hidden_dim)

        


        self.Linear0 = nn.Linear(self.hidden_dim, self.hidden_dim)
        ...
        self.out_proj_1 = nn.Linear(2 * self.hidden_dim, int(self.hidden_dim/2))
    ...
    def forward(self, batched_data):
        ...
        if self.isPE == 1 and self.isSUM == 1:
        
            #size of batched_data = (n,k+1,d+pe)
            all_features = torch.split(batched_data, [batched_data.shape[2]-self.pe_dim, self.pe_dim], dim=2)
            #attribute features 
            att_features = all_features[0]
            #positional encoding
            pe_features =  all_features[1]

            #combine  attribute features and positional encoding
            tensor = self.att_embeddings(att_features) + self.pe_embeddings(pe_features)
