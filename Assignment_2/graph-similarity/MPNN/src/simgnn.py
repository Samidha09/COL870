...
class SimGNN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        #print(pooled_features_1.shape, pooled_features_2.shape)
        scores = self.tensor_network(pooled_features_1, pooled_features_2)
        #scores2 = torch.matmul(torch.transpose(pooled_features_1,0,1), pooled_features_2)
        #print(pooled_features_1.shape, pooled_features_2.shape)
...
class SimGNNTrainer(object):
    ...
    def process_dataset(self):
        ...
        all_graphs = GEDDataset(
            "dataset/{}".format(self.args.dataset), self.args.dataset, train=True
        )
        ...
        #self.number_of_labels = all_graphs.num_features
    ...
    def fit(self):
        ...
        for epoch in epochs:
            ...
            if self.args.plot:
                if epoch % 5 == 0:
                    ...
                    for i, g in enumerate(self.val_graphs[:cnt_test].shuffle()):
        ...
        if self.args.plot:
            ...
            plt.ylabel('Loss')
            print(len(loss_list))
            plt.plot([*range(0, self.args.epochs, 5)], loss_list, label="Train")
            plt.plot(
                [*range(0, self.args.epochs, 5)], loss_list_val, label="Validation"
            )
            plt.legend()
            ...
            plt.ylabel('Loss')
            plt.plot([t*1000 for t in train_time_list], loss_list_val, label="Validation")
            plt.plot([t*1000 for t in train_time_list], loss_list, label="Train")
            plt.legend()
            filename = self.args.dataset
            filename += "_" + self.args.gnn_operator
            if self.args.diffpool:
    ...
    def score_val(self):
        ...
        scores = np.empty((len(self.val_graphs), len(self.training_graphs)))
        ground_truth = np.empty((len(self.val_graphs), len(self.training_graphs)))
        prediction_mat = np.empty((len(self.val_graphs), len(self.training_graphs)))
        k = 5
        rho_list = []
        tau_list = []
        prec_at_10_list = []
        prec_at_20_list = []
        prec_at_k_list = []
        scores_top_k = np.empty((len(self.val_graphs),k))
        t = tqdm(total=len(self.val_graphs) * len(self.training_graphs))

        for i, g in enumerate(self.val_graphs):
            ...
            top_k_ind = np.argpartition(target, -k)[-k:]
            #print(target.shape)
            p1 = prediction[top_k_ind]
            t1 = target[top_k_ind]
            #print(top_k_ind)
            #print(p1.shape, t1.shape)
            scores_top_k[i] = (
                F.mse_loss(p1, t1, reduction="none").detach().numpy()
            )
            scores[i] = (
...
#             )
            prec_at_k_list.append(
                calculate_prec_at_k(k, prediction_mat[i], ground_truth[i])
            )
    ...
    def score_train(self):
        ...
        scores = np.empty((len(self.training_graphs), len(self.training_graphs)))
        ground_truth = np.empty((len(self.training_graphs), len(self.training_graphs)))
        prediction_mat = np.empty((len(self.training_graphs), len(self.training_graphs)))
        k = 5
        rho_list = []
        tau_list = []
        prec_at_10_list = []
        prec_at_20_list = []
        prec_at_k_list = []
        scores_top_k = np.empty((len(self.training_graphs),k))
        t = tqdm(total=len(self.training_graphs) * len(self.training_graphs))

        for i, g in enumerate(self.training_graphs):
            source_batch = Batch.from_data_list([g] * len(self.training_graphs))
            target_batch = Batch.from_data_list(self.training_graphs)
    ...
    def score_test(self):
        ...
        prec_at_20_list = []
        prec_at_k_list = []
        scores_top_k = np.empty((len(self.testing_graphs),k))
...
#        print("p@5: " + str(round(self.prec_at_k, 5)) + ".")
