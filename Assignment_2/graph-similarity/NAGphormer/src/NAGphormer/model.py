...
class GED_MLP(nn.Module):
    def __init__(self, n_layers:int, input_dim:int, hidden_dim:int, dropout:float = 0.) -> None:
        super().__init__()
        kwargs = {"in_features": hidden_dim, "out_features": hidden_dim}
        self.linear_layers = ModuleList([nn.Linear(in_features=input_dim, out_features=hidden_dim)])
        self.linear_layers.extend([nn.Linear(**kwargs) for __ in range(n_layers - 1)])
